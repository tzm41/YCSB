...
import com.xanadu.xanadu.store.DataStore;
import com.xanadu.xanadu.store.DataStoreURI;
import com.xanadu.xanadu.store.KeyValue;
import com.xanadu.xanadu.store.KeyValueStore;
import com.xanadu.jliba.tools.ConfigHandler;
import com.xanadu.xanadu.store.ReferenceTools;
import com.xanadu.xanadu.store.XanaduConnectionFactory;
 ...
 */
public class XanaduLegacyClient extends DB {

  private static final String HOST_PROPERTY = "xanadu.registry";
  ...
  public void init() {
    ...
    String registry = props.getProperty(HOST_PROPERTY);

    InetSocketAddress address = ConfigHandler.getSocketAddress(registry, port, new InetSocketAddress(2020));
    ...
    try {
      keyValueStore = XanaduConnectionFactory.createKeyValueStore(address);
      dataStore = XanaduConnectionFactory.createDataStore(address, STORE_URI, replicas, LOOKUP_LIMIT, USE_CHECKSUM);
    } catch (IOException e) {
  ...
  public Status read(String table, String key, Set<String> fields, HashMap<String, ByteIterator> result) {
    try {
      ...
      HashMap<String, String> stringMap = convertFromBytes(dataStore.readData(kv.getReference()));
